...
// Each layer descriptor carries a LayerAnnotationUncompressed annotation,
// simulating what LayerConvertFunc would stamp before the hook is called.
func buildTestManifest(t *testing.T, cs content.Store, numLayers int) (ocispec.Descriptor, ocispec.Descriptor, []ocispec.Descriptor) {
	...
	for i := range layers {
		body := fmt.Sprintf("overlaybd-test-layer-%d", i)
		...
		layers[i] = ocispec.Descriptor{
			...
			Annotations: map[string]string{
				// Simulate LayerConvertFunc stamping the uncompressed digest
				// (self-referential for OverlayBD blobs).
				LayerAnnotationUncompressed:    dgst.String(),
				LayerAnnotationOverlayBDDigest: dgst.String(),
				LayerAnnotationOverlayBDSize:   fmt.Sprintf("%d", len(body)),
				LayerAnnotationOverlayBDFsType: "ext4",
			},
		}
	}
...
// is collapsed into a bare manifest descriptor.
func TestConvertHookFunc_Index_Flatten(t *testing.T) {
	...
	if images.IsIndexType(result.MediaType) {
		t.Errorf("expected bare manifest, got index type %q", result.MediaType)
	}
...
// and stripped from the written manifest.
func TestConvertHookFunc_Manifest_DiffIDReconcile(t *testing.T) {
	...
	for i, l := range origLayers {
		...
		// LayerAnnotationUncompressed must be stripped from the manifest.
		if _, ok := newMf.Layers[i].Annotations[LayerAnnotationUncompressed]; ok {
			t.Errorf("layer %d: LayerAnnotationUncompressed must be stripped from manifest", i)
		}
	}
}
...
func TestConvertHookFunc_Manifest_WithReferrer(t *testing.T) {
	...
	if newMf.Subject == nil {
		t.Fatal("expected manifest.Subject to be set")
	}
...
// count mismatch is caught and returned as an error.
	...
	cfg := ocispec.Image{
		RootFS: ocispec.RootFS{
			...
			DiffIDs: []digest.Digest{
				...
				digest.FromString("d2"), // mismatch: 2 DiffIDs vs 1 layer
			},
		},
	}
	cfgDesc := writeJSON(t, cs, cfg, ocispec.MediaTypeImageConfig)
...
// TestConvertHookFunc_Helpers verifies the exported helpers IsOverlayBDBlob and
// IsOverlayBDLayer are consistent with the annotation / label semantics used
// by LayerConvertFunc.
func TestConvertHookFunc_Helpers(t *testing.T) {
	t.Run("IsOverlayBDBlob_positive", func(t *testing.T) {
		desc := ocispec.Descriptor{
			Annotations: map[string]string{
				LayerAnnotationOverlayBDDigest: digest.FromString("blob").String(),
			},
		}
		if !IsOverlayBDBlob(desc) {
			t.Error("IsOverlayBDBlob should return true for annotated descriptor")
		}
	})
	t.Run("IsOverlayBDBlob_negative", func(t *testing.T) {
		if IsOverlayBDBlob(ocispec.Descriptor{}) {
			t.Error("IsOverlayBDBlob should return false for empty descriptor")
		}
	})
	t.Run("IsAccelerationLayer_positive", func(t *testing.T) {
		desc := ocispec.Descriptor{
			Annotations: map[string]string{LayerAnnotationAcceleration: "yes"},
		}
		if !IsAccelerationLayer(desc) {
			t.Error("IsAccelerationLayer should return true when annotation is 'yes'")
		}
	})
	t.Run("IsAccelerationLayer_negative", func(t *testing.T) {
		if IsAccelerationLayer(ocispec.Descriptor{}) {
			t.Error("IsAccelerationLayer should return false for empty descriptor")
		}
	})
}

// TestConvertHookFunc_Concurrency_Race launches N goroutines performing separate
// ConvertHookFunc invocations concurrently.  Run with -race.
